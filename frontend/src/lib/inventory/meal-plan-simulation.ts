...
export function buildMealPlanSimulation(
  ...
  const totalStructured = projectionItems.filter(
    (item) => item.requiredTotalQuantity !== null && item.availableQuantity !== null
  );
  const covered = totalStructured.filter((item) => item.status === 'sufficient').length;
  const weeklyCoverage =
    totalStructured.length === 0 ? 0 : Math.round((covered / totalStructured.length) * 100);
    ...
    const ingredients = projectionItems.map((item) => {
      ...
      const consumed =
        item.requiredTotalQuantity === null
          ? null
          : Number((prevConsumed + consumeToday).toFixed(2));
      if (item.requiredTotalQuantity !== null) {
        runningConsumed.set(item.normalizedName, prevConsumed + consumeToday);
      }

      const after =
        before === null || consumed === null
          ? null
          : Number(Math.max(0, before - consumed).toFixed(2));
      ...
      return {
        ...
        consumed:
          consumed === null
            ? null
            : Number(Math.min(consumed, item.requiredTotalQuantity ?? consumed).toFixed(2)),
        after,
  ...
  const missingOrPartialCount = projectionItems.filter(
    (item) => item.status === 'missing' || item.status === 'partial'
  ).length;
  const unknownCount = projectionItems.filter((item) => item.status === 'unknown').length;
  const hasNoInventorySignal = projectionItems.every((item) => (item.availableQuantity ?? 0) <= 0);

  const dailyKitchenMessage =
    hasNoInventorySignal || missingOrPartialCount > 0
      ? `Tu cocina necesita compras para ejecutar este menú (${missingOrPartialCount} ingredientes faltantes o parciales).`
      : unknownCount > 0
        ? `Tu cocina requiere revisión manual en ${unknownCount} ingredientes antes de cocinar.`
