...
export function HomeIntelligenceDashboard(props: Props) {
  return (
    <article className="rounded-2xl border border-[#E8DDD2] bg-white/85 p-3 sm:p-4 xl:p-5">
      ...
      <p className="mt-1 text-sm text-[#6B5A50]">Visión operativa de tu cocina para esta planificación.</p>
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-2">
        <InfoList title="Próximos a agotarse" items={props.criticalIngredients} empty="Sin alertas críticas" />
        <InfoList title="Más reutilizados" items={props.reusedIngredients} empty="Sin reutilización detectada" />
      </div>
    </article>
  );
}
  ...
  return (
    <section className="rounded-xl border border-[#E8DDD2] bg-white/80 p-3">
      ...
      <ul className="mt-2 space-y-1 text-sm text-[#6B5A50]">
        {props.items.length === 0 ? <li>{props.empty}</li> : props.items.map((item) => <li key={`${props.title}-${item}`}>• {item}</li>)}
      </ul>
    </section>
  );
}
