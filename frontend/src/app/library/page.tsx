...
export default function LibraryPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [{ data: tenantRow, error: tenantError }, { data: pdfRows, error: pdfError }, { data: bookRows, error: bookError }] = await Promise.all([
      ...
      const linkedBookIds = new Set(nextPdfs.map((row) => row.tenant_book_id).filter((id): id is string => Boolean(id)));
      const nextBooks = ((bookRows ?? []) as BookRow[]).filter((book) => linkedBookIds.has(book.id));

      setPdfs(nextPdfs);
      setBooks(nextBooks);
    } catch (caughtError) {
      setError(caughtError instanceof Error ? caughtError.message : 'No se pudo cargar biblioteca.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const removePdf = async (pdfId: string) => {
    ...
    try {
      ...
      const { data: pdfRow, error: selectError } = await supabase
        .from('tenant_pdf_library')
        .select('tenant_book_id')
        .eq('id', pdfId)
        .maybeSingle();
      if (selectError) throw selectError;

      const { error: deleteError } = await supabase.from('tenant_pdf_library').delete().eq('id', pdfId);
      ...
      if (pdfRow?.tenant_book_id) {
        ...
        if ((count ?? 0) === 0) {
          const { error: deleteBookError } = await supabase.from('tenant_books').delete().eq('id', pdfRow.tenant_book_id);
      ...
      <section className="mx-auto w-full max-w-5xl rounded-3xl border border-[#E8DDD2] bg-white/80 p-5 premium-shadow">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-[#6B5A50]">Administrá tus PDFs culinarios y libros conectados al RAG.</p>
          </div>
          <div className="flex gap-2">
            <Link href="/recipes/search" className="rounded-xl border border-[#E8DDD2] px-3 py-2 text-sm font-semibold text-[#6B5A50] hover:border-[#C56A1A]/40">
              Abrir buscador
            </Link>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <article className="rounded-2xl border border-[#E8DDD2] bg-white/70 p-4">
            ...
            <p className="mt-1 text-xs text-[#6B5A50]">
              Plan {tenantType === 'professional' ? 'Professional' : 'Home'} · límite {tenantType === 'professional' ? 15 : 5} PDFs.
            </p>
            <ul className="mt-3 space-y-2">
              {pdfs.map((pdf) => (
                <li key={pdf.id} className="rounded-xl border border-[#E8DDD2] bg-white/75 p-3 text-sm">
                  <p className="font-medium text-[#241A14] line-clamp-1">{pdf.storage_path.split('/').pop() ?? pdf.storage_path}</p>
                  <p className="text-xs text-[#6B5A50]">Estado: {pdf.processing_status} · Páginas: {pdf.page_count ?? '-'}</p>
                  <div className="mt-2 flex items-center justify-between text-xs">
                    <span className="text-[#6B5A50]">{new Date(pdf.created_at).toLocaleString()}</span>
                    <button
                      type="button"
                      ...
                      title={canDeletePdf(pdf) ? 'Eliminar PDF' : 'No tienes permiso para eliminar este PDF'}
                    >
                      {workingId === pdf.id ? 'Eliminando...' : canDeletePdf(pdf) ? 'Eliminar' : 'Sin permiso'}
                    </button>
                  </div>
                </li>
              ))}
              {!loading && pdfs.length === 0 ? <li className="text-sm text-[#6B5A50]">No hay PDFs cargados.</li> : null}
            </ul>
          </article>
          ...
          <article className="rounded-2xl border border-[#E8DDD2] bg-white/70 p-4">
            ...
            <ul className="mt-3 space-y-2">
              ...
              {!loading && books.length === 0 ? <li className="text-sm text-[#6B5A50]">No hay libros vinculados.</li> : null}
            </ul>
          </article>
        </div>
