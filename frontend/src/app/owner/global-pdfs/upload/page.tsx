...
async function generateEmbeddingsFromApi(texts: string[]): Promise<number[][]> {
  ...
  if (!response.ok) {
    const payload = (await response.json().catch(() => ({ error: 'Error de embeddings.' }))) as {
      error?: string;
    };
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const chunkCandidates = (
        chunks.length > 0 ? chunks : [{ content: `Documento global: ${title}`, pageNumber: 1 }]
      ).slice(0, 160);
      ...
      formData.set('pageCount', String(Math.max(pageCount, 1)));
      formData.set(
        'chunks',
        JSON.stringify(
      ...
      const uploadResponse = await fetch('/api/owner/global-pdfs', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!uploadResponse.ok) {
        ...
        throw new Error(payload.error ?? 'No se pudo subir PDF global.');
      }

      const uploadPayload = (await uploadResponse.json()) as { chunkCount: number };

      setMessage(`Global PDF cargado e indexado: ${uploadPayload.chunkCount} chunk(s).`);
    ...
    <section className="space-y-4">
      <article className="rounded-3xl border border-[#E8DDD2] bg-white/90 p-5 premium-shadow">
        ...
        <p className="mt-1 text-sm text-[#6B5A50]">
          Subí contenido global por región culinaria y dejalo listo para búsqueda contextual.
        </p>
      </article>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <label className="space-y-1 text-sm">
            ...
            <input
              ...
              placeholder="Ej: Cocina mediterránea familiar"
              className="h-11 w-full rounded-xl border border-[#E8DDD2] bg-white px-3"
            />
          </label>
          <label className="space-y-1 text-sm">
          ...
          <label className="space-y-1 text-sm">
            <span className="inline-flex items-center gap-2 font-medium text-[#241A14]">
              <Globe2 size={14} /> Región culinaria
            </span>
            <input
              value={region}
              onChange={(event) => setRegion(event.target.value)}
              placeholder="Iberoamericana"
              className="h-11 w-full rounded-xl border border-[#E8DDD2] bg-white px-3"
            />
          </label>
          <label className="space-y-1 text-sm">
          ...
          <label className="space-y-1 text-sm">
            ...
            <input
              ...
              placeholder="Casera / Gourmet / Tradicional"
              className="h-11 w-full rounded-xl border border-[#E8DDD2] bg-white px-3"
            />
          </label>
          <label className="space-y-1 text-sm">
            ...
            <input
              ...
              placeholder="tradicional, familia, sopas"
              className="h-11 w-full rounded-xl border border-[#E8DDD2] bg-white px-3"
            />
          </label>
          <label className="space-y-1 text-sm md:col-span-2">
            ...
            <textarea
              ...
              placeholder="Contexto del libro y valor para la plataforma..."
              className="min-h-[110px] w-full rounded-xl border border-[#E8DDD2] bg-white px-3 py-2"
            />
          </label>
        </div>
        ...
        <div className="mt-4 rounded-2xl border border-dashed border-[#E8DDD2] bg-[#FAF6F1] p-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#E8DDD2] bg-white px-3 py-2 text-sm font-semibold text-[#6B5A50] hover:border-[#C56A1A]/35"
            >
