...
export async function POST(request: NextRequest) {
  ...
  const peopleCount = typeof body.peopleCount === 'number' && Number.isFinite(body.peopleCount) && body.peopleCount > 0
    ? body.peopleCount
    : 4;
  const inventory = (body.inventory ?? []).filter(Boolean).slice(0, 80).join(', ') || 'No especificado';
  const restrictions = (body.restrictions ?? []).filter(Boolean).slice(0, 20).join(', ') || 'No especificado';
  const preferred = (body.profile?.preferred ?? []).filter(Boolean).slice(0, 12).join(', ') || 'No especificado';
  const goals = (body.profile?.goals ?? []).filter(Boolean).slice(0, 12).join(', ') || 'No especificado';
  const level = body.profile?.level?.trim() || 'No especificado';

  if (!menuContent) {
    serverLogger.warn('inventory_suggestion.invalid_input', { requestId, reason: 'missing_menu_content' });
...
- No incluyas texto fuera del JSON.`;
  ...
  if (!response.ok) {
    ...
    return NextResponse.json({ error: `No se pudo calcular inventario sugerido: ${errorText}` }, { status: 502 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'No se pudo parsear JSON de inventario sugerido.' }, { status: 502 });
  }
}
