...
describe('Gemini DTO guards', () => {
  it('accepts valid embed response shapes', () => {
    ...
    expect(isGeminiBatchEmbedContentsResponseDto({ embeddings: [{ values: [0.1] }, { values: [0.2] }] })).toBe(true);
  });

  it('rejects invalid embed response shapes', () => {
    expect(isGeminiEmbedContentResponseDto({ embedding: { values: ['0.1'] } })).toBe(false);
    expect(isGeminiBatchEmbedContentsResponseDto({ embeddings: [{ values: [0.1] }, { values: null }] })).toBe(false);
  });
